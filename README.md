...
答题后会出现 **“🧠 理解记忆”** 卡片，帮助把词义和词形、词源线索连起来：

- 只对有较可靠依据的词做明确拆解，例如 `circumspect = circum（周围） + spect（看）`；
- 对能识别出常见词根、但不适合做完整词源拆解的词，只显示“词根关联提示”；
- 对不适合拆的词明确提示“不建议硬拆”，避免为了记忆制造错误词源；
- 会结合中文核心义、近义词和已有英文释义做理解；
- 提供 Etymonline / Wiktionary 外部查证入口；
- 默认只在作答后显示，不会提前泄题；
- 可在设置中关闭。

当前内置了大量 GRE 高频词的保守词源/词形说明，并覆盖 `cred / culp / dict / voc / loqu / spect / aud / script / tract / duct / gress / port / gen / mort / chron / bene / mal / equi` 等常见词根族。

### 记忆规则

词汇题现在支持三种方向，可在词汇页顶部或设置中随时切换：

- **双向混合（默认）**：随机出现“英文 → 选中文”和“中文 → 选英文”；
- **英文 → 中文**：传统释义识别；
- **中文 → 英文**：强化主动回忆，题干给中文义，四个选项都是英文；作答后四个英文选项都会展开各自中文意思，错误干扰项也会显示释义。

中文 → 英文模式不会在作答前自动朗读目标词，避免发音直接泄露答案；作答后会自动朗读正确英文，之后可按 Tab 重读。
...
设置中新增 **GRE 考试日期**。设置后系统会把考试日换算成剩余备考天数，并显示当前阶段：
...
Quant starter bank 现在按 ETS Diagnostic Service 的四大内容领域标注：
...
同时记录题型、topic、Pure/Real-life setting 与 difficulty。做错 Quant 后可以标记主要错误原因：
...
统计页会显示各内容领域正确率/平均用时，以及错误原因分布。

### GRE Quant 练习计算器

Quant 页面新增练习计算器，支持基本四则运算、平方根、正负号和 MC/MR/M+/M-。这是用于练习考试中的计算器决策与操作节奏，不代替心算/估算训练。

### 启动自检

启动时检查：

- 词汇/Verbal/Quant ID 是否重复
- Quant 答案索引是否合法
- Quant ETS 分类字段是否完整
- 四大 Quant 内容领域是否有覆盖
- localStorage 是否可用

本轮同时修复了 Quant starter bank 中 q10 的答案键错误。

## 可编辑完整 List 计划表

“大三千 List 复习计划”的完整计划表现在支持手动修改：

- 点击 **编辑完整计划表**；
- 可直接修改每一行的日期、上午新学内容、下午/晚上复习内容；
- 可 **新增一天** 或删除任意一行；
- 点击 **保存完整计划** 后，手动计划持久保存在浏览器本地；
- 手动计划存在时会显示“手动”模式；
- 可随时点 **恢复自动计划**，重新回到 0/1/3/7/14/29 天复习间隔生成的自动表；
- 如果在设置中修改开始日期、每天 List 数或总 List 数并选择“重新生成自动计划”，系统会先提示是否清除手动修改。

旧版 `gre-17day-plan-v1` 设置会自动迁移到新版 `gre-17day-plan-v2`。
